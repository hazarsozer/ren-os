...
5. `lib.ingest()` still does not call `lib/adapter/claude_md.py`'s `write_project_claude_md(repo_root, project_slug)` — wired as of 0.4.3, but only into `bootstrap-project`'s `bootstrap()`, which owns the `<repo_root>/CLAUDE.md` stamp. No `CLAUDE.md` pointer block is written by this skill today.
