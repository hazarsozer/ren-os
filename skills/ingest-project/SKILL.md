...
5. **Not wired yet:** `lib/adapter/claude_md.py`'s `write_project_claude_md(repo_root, project_slug)` exists and is unit-tested — it would stamp the thin RenOS pointer block (managed `ren:` markers) into `<repo_root>/CLAUDE.md`, pointing at the project's L2 map plus the recall-doctrine reminder — but `lib.ingest()` does not call it as of 0.4.2. No `CLAUDE.md` pointer block is written by this skill today.
...
- Modify anything in the scanned project. `scan_repo` is read-only, full stop — see `scan.py`'s own INVARIANTS block. (The `CLAUDE.md` marker-block write described in step 5 is not wired yet — see that step's note.)
