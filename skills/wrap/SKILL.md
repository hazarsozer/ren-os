...
3. **Call `skills.wrap.lib.wrap_session(narrative_md, durable_items, session, verdicts=..., cwd=Path("<absolute session working dir>"))`.** (`llm_call=...` remains supported for callers with a live callable — see step 3.4 below — but `verdicts=...` is the standard kwarg for the batched-subagent path this SKILL drives.)
   ...
   - **Classify via ONE classifier-class subagent (batched).** When there are
     ...
     1. Build the per-item prompts mechanically — including the project's
        ...
        Pass `taxonomy_block=None` when there is no taxonomy object at all
        (no project, or `schema.md` failed to load/parse — blind), and
        `taxonomy_block=render_block(tax)` whenever one loaded — including
        when it parsed to zero nodes (`""`), which the classifier reads as
        "defined but empty, propose a new root" rather than "no taxonomy":
        ...
        if project:
            try:
                ...
                tax = None
        tb = render_block(tax) if tax is not None else None
