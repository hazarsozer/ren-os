...
description: |
  ...
  preferences / Opinions+non-goals / Contribution), writes
  ~/.startup-framework/wiki/identity.md (hybrid YAML frontmatter + markdown
  body), and pushes a public summary to the Activity Feed at
  identities/<handle>.md via feed.upsert_identity(). Detects existing
  identity.md and offers full-refresh vs specific-field updates.
version: 0.1.0
license: MIT
...
contract:
  required_outputs:
    ...
    - "Public summary commit/push via feed.upsert_identity(handle, public_md) — best-effort, see graceful-degradation"
  budgets:
    turns: 36                  # ~2 per question (ask + capture); generous slack
    files_written: 2           # identity.md + (delegated) feed identities/<handle>.md
  ...
  permissions:
    ...
    write:
      - "~/.startup-framework/wiki/identity.md"
    execute: []
    # feed.upsert_identity() is a delegated call, owned by sf-feed; its write
    # permission lives in that skill, not here.
  completion_conditions:
    ...
    - "If feed is reachable: feed.upsert_identity returned committed=true OR explicit user-acknowledged skip"
  output_paths:
    - "~/.startup-framework/wiki/identity.md"
    - "~/.startup-framework/activity-feed/identities/<handle>.md  # via feed.upsert_identity"
...
references_on_demand:
  ...
  - "references/public-summary-format.md"
---
...
**Handle prepopulation (Stage-4 invocation case):** when `/sf:install` Stage 4 runs this skill, the orchestrator has already collected a tentative handle in its Stage 3 mini-prompt. The install-state checkpoint surfaces it via `proposed_handle`. The interview pre-fills Q1's handle answer with that value and asks the friend to confirm or change. If changed, the orchestrator handles the feed-side rename per the Stage 3/4 ordering doc.
...
### 4. Sync public summary to Activity Feed

Load `references/public-summary-format.md` for the subset spec.

Render the public summary (frontmatter subset + intro paragraph + "What I contribute" paragraph). Call:

```
feed.upsert_identity(handle, public_identity_md) -> WriteResult
```

Idempotent — only commits if content changed. On failure:

- Network/git error → log warning, do NOT abort the interview. The local identity is the source of truth; the public sync is best-effort.
- Handle collision detected by feed (joiner case) → return to Q1, re-prompt for handle, retry.
- Schema-version mismatch → surface as a migration hint, recommend `/sf:doctor`.
...
✓ public summary pushed to <feed-url>/identities/<handle>.md
  (or)
⚠ public summary not pushed (feed unreachable); will retry on next /sf:wrap or /sf:interview
...
- **Don't block on feed push.** Per the graceful-degradation rule from ADR-022 + my answer to feed-2's contract: identity.md is the source of truth; feed sync is best-effort.
...
- Public-subset test: feed.upsert_identity receives a strict subset of local identity fields
