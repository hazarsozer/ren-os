...
- Multi-line text escaped via `\n` literal substitution rather than raw newlines: preserves the "one bullet per pin" invariant so `/sf:wrap`'s parser doesn't have to handle multi-line bullets.
...
- ADR-009 (Consolidate via /wrap) — this skill is the companion `/sf:note` named there
