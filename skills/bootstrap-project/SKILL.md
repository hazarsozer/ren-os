...
3. **Not wired yet:** `lib/adapter/claude_md.py`'s `write_project_claude_md(repo_root, project_slug)` exists and is unit-tested — it would stamp the thin RenOS pointer block (managed `ren:` markers) into `<repo_root>/CLAUDE.md` — but neither `bootstrap()` nor `ingest-project`'s `lib.ingest()` calls it as of 0.4.2. The only per-repo pointer file actually written today is `AGENTS.md` (step above).
