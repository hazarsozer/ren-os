...
2. Call `importlib.import_module("skills.bootstrap-project.lib").bootstrap(project_slug, session, repo_root=repo_root)`:
   ...
   - Stamps the manifest's `project` profile under `projects/<slug>/`: `overview.md` (the guaranteed wake-up orientation page) and, as of 0.6.2, the empty `knowledge/` directory — the sanctioned home for this project's durable distilled pages (`type: project-knowledge`, `schema_version: 1`, `project: <slug>`). Bootstrap only *creates* the directory; `/ren:ingest-project` and later sessions fill it, and the L2 map's Decision-map pointers index it. Project-specific pages must NOT go in root-level `decisions/`·`patterns/`·`research/` — that's the promotion-gated instruction plane (`docs/decisions/2026-08-01-project-knowledge-subtree.md`).
