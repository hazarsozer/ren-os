...
name: metric-watch
description: |
  The §3.5 minimal metric-watch routine. Watches six signals — injection
  budget growth, memory growth rate, classifier fail-closed events,
  no-LLM-with-candidates defects, fan-out silence, and backup configuration — and writes
...
Spec §3.5's minimal metric-watch: "one routine watches budget ceiling, memory growth rate, classifier fail-closed events, backup unconfigured and writes findings to the journal for the next wake-up." Six independent checks (the post-0.8.0 `no_llm-with-candidates` signal joined the original four; the 0.8.7 `fan-out-silent` signal made six), one journal line per run.
...
2. `watch` runs all six checks (see table above), each wrapped so a crash becomes a `check-error` finding rather than an exception.
