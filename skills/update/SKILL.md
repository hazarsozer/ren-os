---
name: update
description: Use when the user runs /ren:update to upgrade the framework. Drives the migration state machine — fetches latest version, classifies the bump, snapshots the wiki, runs migrations, verifies via verify.json, shows diffs for approval, applies, and re-verifies. Snapshot/rollback is built in. Never silent on MAJOR bumps.
version: 0.1.0
license: MIT
type: skill
schema_version: 1
...
contract:
  required_outputs:
    ...
    - "A post-update /ren:doctor --post-update report; on new issues, an explicit no-auto-rollback message naming the snapshot path"
  ...
  permissions:
    ...
    execute:
      ...
      - "gh (read-only: gh api repos/<org>/ren-os/contents/.claude-plugin/plugin.json)"
...
```
/ren:update                           upgrade to latest stable (interactive)
/ren:update --rc                      upgrade to latest RC (subscribes to ren-os-rc if needed)
/ren:update --to v1.2.5               specific version (downgrade if a snapshot exists)
/ren:update --dry-run                 show plan; no writes
/ren:update --auto                    auto-approve MINOR additive migrations (still prompts on MAJOR)
/ren:update --restore-snapshot        interactive picker; restore wiki from a snapshot (per ADR-023 V1 fence — flag, NOT a new /ren:rollback command)
```
...
```
   ...
   └──────┬──────┘  Skipped → stays at old schema; /ren:doctor will keep flagging
   ...
   │             │  If wiki has a git remote AND user previously ran /ren:backup: commit + push (best-effort, never blocks).
   └──────┬──────┘
          ▼
   ...
   └──────┬──────┘  Per lead's pushback: explicit message:
          ...
          │            Inspect at: <paths>.
          │            Run /ren:update --restore-snapshot if you want to revert."
          ▼
       ┌──────┐
       │ DONE │
       └──────┘
...
| Post-update doctor issues | Migration applied; new warnings/errors surfaced. **No auto-rollback** (per lead). User decides. | "Migration applied but post-update doctor shows N issue(s). Inspect at: <paths>. Run /ren:update --restore-snapshot if you want to revert." |
...
- **Manual restore recipe** (printed by `/ren:update --restore-snapshot`):
  ```bash
...
- Running `/ren:update` twice in a row is a no-op the second time (PLANNING reports "no migrations needed; already at v<latest>", exits).
...
- If user is on stable and runs `/ren:update --rc`: prompt to add `ren-os-rc` marketplace (if not yet added), then install from RC. CC treats it as the same plugin name; under the hood, this swaps which marketplace the plugin is pinned to.
- If user is on RC and runs `/ren:update` (no flag): if latest stable >= installed RC, prompt to switch back to stable; otherwise stay on RC and update to latest RC.
...
- `/ren:update`'s VERIFYING_FINAL state invokes `/ren:doctor --post-update`.

### With sf-onboarding (`onboarding-2`)
- After `/ren:install --restore`, the friend may need to immediately `/ren:update` if their wiki is at older schemas. Handled naturally by the state machine (PLANNING computes the chain).

### With sf-lifecycle (`lifecycle-2`)
- Wake-up hook must be **safe to re-register across `/reload-plugins`** — `/ren:update` calls `/reload-plugins` programmatically when commands/skills/hooks change.
