...
5. Render the results to the friend (page path + relevant excerpt/full content, caller's choice of formatting) — include each hit's `trust` label (e.g. `[trust: model]`, `[trust: foreign]`) alongside its page path, so the friend can see at a glance which results are the framework's own writes versus foreign/ingested content.
