...
amendments:
  ...
  - "2026-06-28 (B1): the loop now persists its outcomes. `/ren:improve-skill`'s close-out appends each run to a project-scoped `experiment-log.md` (the forward-declared ADR-037 page-type; writer `skills/improve-skill/lib/experiment_log.py`). This is the loop's compounding memory + the supervised-run audit trail ADR-036 requires before earning autonomy. Append-only, ungated, no schema bump (page-type already v1); the orchestrator is unchanged — the write is a SKILL.md close-out step."
---
