...
def test_schema_template_stamps_empty_taxonomy_fence():
    ...
    taxonomy` parses — so a freshly bootstrapped project's concept routing
    is defined-but-EMPTY (I4, spec §6 ruled authoritative) rather than
    blind, crashing, or silently guessing a placement the first time a
    concept-kind durable item shows up. The fence is deliberately empty
    until a session adds branches: `parse_taxonomy` parses it cleanly to
    `Taxonomy(nodes=())` — it does NOT raise, and concept routing is NOT
    reported blind (`concept_routing.blind` stays `None`) for a project
    whose schema.md looks exactly like this stub."""
    from lib.memory.taxonomy import Taxonomy, parse_taxonomy
    ...
    tax = parse_taxonomy(template_text)
    assert tax == Taxonomy(nodes=())
