...
Per ADR-017, the plugin ships a fresh empty skeleton — never the framework's
own development wiki content. Templates must hold only structural scaffolding
and the small placeholder-variable set documented in `manifest.yaml`. This
lint walks every file under `templates/` and asserts no forbidden substring
appears.
...
-----
    ...
    # (wiki-skeleton/templates/ + every skills/*/templates/)
    python3 wiki-skeleton/tests/lint_no_dev_wiki_content.py
    # exit 0 = clean, exit 1 = hits found

Or pass one or more explicit template roots:

    python3 wiki-skeleton/tests/lint_no_dev_wiki_content.py \
        path/to/templates [more/templates ...]
...
Wiring into CI
--------------
We don't have CI yet. When we add it (likely GitHub Actions on the
marketplace repo), add a step that runs `python wiki-skeleton/tests/
lint_no_dev_wiki_content.py` from the repo root. Until then: run it locally
before every PR that touches `wiki-skeleton/`.
"""
...
def walk_templates(templates_root: Path) -> list[Path]:
    """Return every regular file under templates/. Excludes .gitkeep
    (zero-byte by convention) only when its content is empty."""
    if not templates_root.is_dir():
        raise NotADirectoryError(
            f"Templates root not found at {templates_root}. "
            "The lint expects wiki-skeleton/templates/ to exist."
        )
...
def scan_file(file_path: Path, substrings: list[str]) -> list[Hit]:
    ...
    try:
        ...
        # Binary file inside templates/. Not expected; flag with a synthetic
        # hit so a reviewer notices.
        return [
            Hit(
                file_path=file_path,
                line_number=0,
                column=0,
                substring="<binary file in templates/>",
                line_text="",
            )
        ]
...
def repo_root() -> Path:
    """Repo root inferred from this script's location: <repo>/wiki-skeleton/tests/…"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    repo = repo_root()
    forbidden_file = repo / "wiki-skeleton" / "tests" / "forbidden-substrings.txt"
