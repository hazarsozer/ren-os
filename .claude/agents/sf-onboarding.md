...
4. `wiki/decisions/017-per-friend-wiki-scope.md` — load-bearing: wiki is per-friend-local; ship skeleton, NOT content
5. `wiki/decisions/020-joiner-and-leaver-experience.md` — Stage 3 detects existing Activity Feed for joiners
6. `docs/superpowers/specs/2026-05-28-startup-framework-design.md` §5 (Onboarding)

## Hard constraints

- The friend's wiki starts EMPTY. Do not seed it with any content from our development wiki. (ADR-017)
- `/sf:interview` output is hybrid YAML frontmatter + markdown body. Public summary goes to `<activity-feed>/identities/<handle>.md`; full identity stays in friend's local `wiki/identity.md`. (ADR-022)
...
- Stage 3 must DETECT existing Activity Feed repos (joiner case) vs bootstrap fresh (first-friend case). Coordinate with sf-feed on the detection contract.
...
- With sf-feed: Stage 3 detection-vs-bootstrap decision tree + identities/<handle>.md format
...
5. Plug points where sf-feed (Stage 3) and sf-distribution (install shell handoff) need to integrate
