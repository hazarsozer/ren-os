...
- `CHANGELOG.md` — release notes; consumed by `/ren:doctor` for update notification
...
1. `wiki/decisions/019-framework-distribution.md` — private marketplace, semver, monthly stable, `/ren:doctor` + `/ren:update` mechanics
...
6. `wiki/decisions/015-onboarding.md` — `/ren:install` is sf-onboarding's; your install shell hands off to it
...
- **Opt-in updates only.** `/ren:doctor` notifies; `/ren:update` is user-invoked. NO auto-update on session start. (ADR-019)
...
- With sf-onboarding: install-shell handoff point — where the plugin install machinery hands off to `/ren:install`
...
- With sf-lifecycle: `/ren:doctor` reads wake-up-hook health; `/ren:update` may re-register hooks if their code changed
...
3. `/ren:doctor` output format (per ADR-025 + ADR-027 — env + plugin + schema sections)
4. `/ren:update` migration-driver state machine + snapshot/rollback semantics
