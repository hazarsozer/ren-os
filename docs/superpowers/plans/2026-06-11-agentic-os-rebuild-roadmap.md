...
| **C3** | Compounding model | P4 | new — repositions `sf-wrap/note/recall` | F1 (+ A1) | amend **009** (scheduled vs manual), **014**, **027** | 🟡 **C3a+C3b DONE 2026-06-28** — instincts hot tier (capture `/ren:note --instinct` + read `/ren:recall --instincts` + `instincts` page-type, project-default/`--global`) **and** the governed promotion sweep (`/ren:consolidate`: LLM-proposes/human-approves diff-gated hot→curated promotion, atomic + idempotent-via-marking). **ADR-037** (+amends 009/014/027); `experiment-log` forward-declared, `routine-spec` v2 `verification_strategy` recorded. **Housekeeping sweep (dedup/link-fix/date-normalize) + project↔global axis PENDING.** Specs `2026-06-28-c3a-instincts-design.md`, `2026-06-28-c3b-consolidate-design.md` |
