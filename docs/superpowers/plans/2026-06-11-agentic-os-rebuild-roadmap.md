...
| **C3** | Compounding model | P4 | new — repositions `sf-wrap/note/recall` | F1 (+ A1) | amend **009** (scheduled vs manual), **014**, **027** | 🟡 **C3a+C3b+C3c + project↔global axis DONE 2026-06-28** — instincts hot tier (capture `/ren:note --instinct` + read `/ren:recall --instincts` + `instincts` page-type, project-default/`--global`), the governed promotion sweep (`/ren:consolidate`: LLM-proposes/human-approves diff-gated hot→curated promotion, atomic + idempotent-via-marking), **and the dead-link repair housekeeping sweep (`/ren:consolidate --fix-links`: deterministic conservative link-fix, gate-per-repair/apply-per-file, naturally idempotent)**. **ADR-037** (+amends 009/014/027); `experiment-log` writer shipped (B1, 2026-06-28; `wiki/projects/<p>/experiment-log.md` via the improve-skill close-out), `routine-spec` v2 `verification_strategy` shipped (C2, 2026-06-28 — the framework's first real schema migration: `migrations/routine-spec-1-to-2/` + the discovery-glob fix). **Project↔global axis shipped (`/ren:consolidate --to-global`, 2026-06-28). Housekeeping remainder (dedup/contradiction-prune) PENDING — high-risk LLM-semantic, recommend-defer.** Specs `2026-06-28-c3a-instincts-design.md`, `2026-06-28-c3b-consolidate-design.md`, `2026-06-28-c3c-link-repair-design.md`, `2026-06-28-c3-project-global-axis-design.md` |
...
| **H2** | Multi-agent glue + lightweight-skill tier + broadened onboarding | P2/glue | extend `sf-interview/install` + `CLAUDE.md` | A1 | amend **011** (lightweight tier) | ✅ **DONE 2026-06-28** (parts 1+2 2026-06-27; part 3 2026-06-28) — `tier: lightweight` carve-out (ADR-011 amendment; not self-improvable + doctor-lint exempt) + `agent-orchestration.md` reference (decomposition tree + model-routing) via `/ren:cadence`. **Part 3 (broadened onboarding) DONE 2026-06-28** — optional venture arc (Section F) on `/ren:interview` + the new loose `venture-profile` page-type (5 pages: company/market/icp/team/brain-dump under `wiki/venture/`); `identity.md` unchanged (additive, no migration). Spec `2026-06-28-h2-broadened-onboarding-design.md`. |
