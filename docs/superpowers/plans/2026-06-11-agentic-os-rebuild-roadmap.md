...
| **C3** | Compounding model | P4 | new — repositions `sf-wrap/note/recall` | F1 (+ A1) | amend **009** (scheduled vs manual), **014**, **027** | 🟡 **C3a DONE 2026-06-28** — instincts hot tier (**ADR-037**): `/ren:note --instinct` capture + `/ren:recall --instincts` + `instincts` page-type, project-default/`--global` routing; ADR-009/014/027 amended; `experiment-log` forward-declared, `routine-spec` v2 `verification_strategy` recorded. **C3b (governed hot→curated promotion/dedup sweep) PENDING.** Spec `docs/superpowers/specs/2026-06-28-c3a-instincts-design.md` |
