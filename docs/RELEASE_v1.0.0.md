...
(Activity Feed removed — ADR-031, solo-first pivot. There is no feed announcement; out-of-band notification + `CHANGELOG.md` + `/ren:doctor` carry "what shipped".)
...
RenOS's first ship. A private Claude Code framework for a friend group building startups together.
...
- **Self-improving skills** — `/ren:improve-skill` runs the Karpathy auto-research loop with four safety primitives (per ADR-012).
- **One-command install** — `/ren:install` walks 7 stages in ~10 minutes.
- **One-command verify** — `/ren:doctor` reports env + plugins + schemas + update + backup status in one screen.
- **Opt-in updates** — `/ren:update` is user-invoked; never automatic. Snapshot before every migration. Latest 3 retained.
...
- **Opt-in updates.** `/ren:doctor` notifies; you decide when to upgrade.
- **N+3 schema deprecation window.** Pages stay readable for 3 framework versions after a schema bump.
- **Pre-release channel (RC).** Friends who want early access subscribe to `ren-os-rc`; dogfood for a week before stable.
...
- A `/ren:rollback` slash command (use `/ren:update --restore-snapshot` flag instead)
...
> A private Claude Code framework for our friend group. Installs in 10 minutes via `/plugin marketplace add <org>/ren-os`. Per-person local wiki + opt-in updates. Try `/ren:install` when you have a quiet hour.

### Option B (memory-pain-led)

> If you're tired of re-explaining your context to Claude every session, this framework fixes it. 10-minute install. Your wiki stays on your machine — yours alone. Try it: `/plugin marketplace add <org>/ren-os` → `/ren:install`.

### Option C (cinematic)

> v1 of the framework is up. The bet: Claude is sharper when it remembers you. Install: `/plugin marketplace add <org>/ren-os` → `/ren:install`. 10 minutes, then run a session normally and tell me what felt different.
...
- After tagging: notify friends out-of-band. The `CHANGELOG.md` entry + `/ren:doctor`'s update notice are the canonical "what shipped" record (Activity Feed removed — ADR-031).
- Friends with `userConfig.rcChannel = true` will continue tracking ren-os-rc. They're already running RC-equivalent content; the stable tag is their cue to switch back to the stable marketplace if they want.
