...
Counts: 26 sites — `queue-mediated` 3, `additive` 1, `confirmed` 3,
`scratch/state` 18, **`VIOLATION` 1 (fixed)**.
...
| 24 | `migrations/project-knowledge-1/migrate.py::main` (+ `rewrite_pointers`) | `write_text` + `unlink` | confirmed | Issue #20: relocates flat `projects/<slug>/*.md` pages into `knowledge/` and rewrites the L2 map's pointers. **Outside the enforced `lib`+`skills` sweep** (it lives in `migrations/`, like `trust-backfill-1`), registered here anyway because it moves user knowledge. `confirmed`: dry-run is the DEFAULT — nothing is written without an explicit `--apply` from a human running the script by name. Never overwrites: an existing `knowledge/<name>.md` is a reported COLLISION and the flat file is left in place, so the `unlink` only ever follows a successful write of the same bytes to a path that did not exist. Body and `ren_*` frontmatter are preserved byte-for-byte; every move is journaled to `state_dir()/migrations/project-knowledge-1.jsonl`. The pointer rewrite is a line-level substring replacement inside `## Decision map` on `type: l2-map` pages only. |
| 25 | `skills/wiki-health/lib/watermark.py::advance_watermark` | `write_text` + `os.replace` | scratch/state | 0.6.5 Task 2: `state_dir()/wiki_lint_watermark.json` — how far incremental wiki-lint has progressed through the journal (`journal_lines_seen`, `clean`, `stamped_at`). Whole-file atomic replace, not a merge, but the content is a single disposable stamp with no per-key history to preserve; a corrupt/missing file degrades to "lint everything from the start" (`read_watermark()` returns `{}`) rather than raising. Nothing user-authored lives at that path. |
| 26 | `skills/update/lib/__init__.py::gc_stale_envs` | `shutil.rmtree` | scratch/state | Issue #40: removes `framework_root()/.envs/<version>` dirs — per-version `UV_PROJECT_ENVIRONMENT` targets, never anything a human wrote. Only removes a version dir when its name is absent from the live plugin cache listing (`$CLAUDE_PLUGIN_ROOT`'s parent); an unresolvable cache root or missing `.envs` dir is a no-op rather than a guess, and a per-dir `OSError` skips just that directory. Losing one costs an `uv sync` re-warm, nothing else. |
| 27 | `skills/distill/lib/__init__.py::write_watermark` | `write_text` + `os.replace` | scratch/state | Task 4, 0.7.x knowledge-flow train: `state_dir()/distiller-watermark.json` — how far `/ren:distill` has consumed the L1 journal (`{"ts": <iso>}`), same shape and same `os.replace` pattern as row 25's wiki-lint watermark. Whole-file atomic replace, one disposable stamp with no per-key history; a corrupt/missing file degrades to `read_watermark() -> None`, which `l1_batch(None)` treats as "process every L1 from the start" rather than raising. Nothing user-authored lives at that path, and the watermark only ever advances via an explicit `write_watermark` call — never auto-advanced by this lib. |
