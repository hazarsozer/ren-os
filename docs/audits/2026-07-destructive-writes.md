...
| 22 | `lib/instrument/calibration.py::_write_text_atomic` | `write_text` + `os.replace` | scratch/state | `state_dir()/metrics/last_injection.txt` — the scratch pairing file wake-up overwrites each session (0.6.1 E5a). Overwrite IS the contract (it holds exactly one session's injected payload, never a history); nothing user-authored can live at that path, and losing it costs at most one skipped calibration. |
